...
void Solve(int tc) {
    ...
    vector<string> res(lenS, "");

    int i = 0, j = 0;

    int resLen = 0, resIdx = 0;
    while (i < lenS && j < lenP) {
        if (resLen) res[resIdx].push_back(s[i]);

        if(s[i] == p[j]) {
            resLen++;
            if (resLen == 1) res[resIdx].push_back(s[i]);
            i++;
            j++;
            if (j == lenP && lenS - i >= lenP) {
                j = 0;
                resIdx++;
            }
            ...
            if (resLen) resLen++;
            resIdx++;
            if (s[i] == p[j - 1]) res[resIdx] = p[j - 1];
            i++;
        }
    }

    SORT_UNIQUE(res);
    string ultimateResult = res[1];
    debug(ultimateResult);

    if (ultimateResult.length() == 0) cout << "AI has no idea" << nl;
    else cout << ultimateResult << nl;
}
